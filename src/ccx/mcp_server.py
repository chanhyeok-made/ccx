...
from ccx.compactor import (
    load_compaction_summary,
    check_context_fill,
    get_recent_conversation_text,
    save_compaction_summary as _save_compaction_summary,
    CompactionSummary,
)
...
@mcp.tool()
def get_compaction_context(transcript_path: str) -> dict:
    """Get current context fill level and recent conversation text for compaction.

    Args:
        transcript_path: Path to the session transcript JSONL file.

    Returns:
        Dict with fill_pct, model, conversation_text, and session_id.
    """
    fill_pct, model = check_context_fill(transcript_path)
    conversation_text = get_recent_conversation_text(transcript_path)
...
def save_compaction_summary(
    ...
    """Save a compaction summary generated by the LLM agent.

    Args:
        project_dir: Project root directory path.
        summary: LLM-generated summary of the session context.
        changed_files: List of file paths that were modified.
        key_decisions: List of key user decisions made in the session.
        pending_tasks: List of incomplete tasks.
        context_fill_pct: Current context fill percentage.
    ...
    path = _save_compaction_summary(cs, project_dir)
